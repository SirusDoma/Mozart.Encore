...
public sealed partial class CommandDispatcher : ICommandDispatcher
{
    ...
    ICommandDispatcher ICommandDispatcher.Map<TSession, [DynamicallyAccessedMembers(DynamicallyAccessedMemberTypes.All)]
        TController>(Func<TSession, TController> factory)
    {
        MapRoute(factory);
        return this;
    }
    ...
    private void MapRoute<TSession, [DynamicallyAccessedMembers(DynamicallyAccessedMemberTypes.All)] TController>(
        Func<TSession, TController> factory
    )
        ...
        where TController : CommandController
    {
        ...
        foreach (var method in methods)
        {
            ...
            foreach (var attribute in method.GetCustomAttributes<CommandHandlerAttribute>())
            {
                ...
                var descriptor = new CommandHandlerDescriptor(method, attribute);
    ...
    private async Task<CommandResponse> ExecuteHandlerAsync(CommandHandler handler, Session session, Enum command,
        IMessage? request, CancellationToken cancellationToken)
    {
        ...
        foreach (var filter in handler.Filters)
        {
            ...
            {
                return await FilterExceptionAsync(
        ...
        try
        {
            message  = await handler.ExecuteAsync(session, request, cancellationToken).ConfigureAwait(false);
    ...
    private async Task<CommandResponse> FilterExceptionAsync(CommandExceptionContext context, CancellationToken cancellationToken)
    {
        bool propagate = true;
    ...
    private async Task WriteFrameAsync(Session session, CommandResponse response, CancellationToken cancellationToken)
    {
        await Task.WhenAll(
            response.Frames.Select(frame => session.WriteFrameAsync(frame.Payload, cancellationToken))
        );
    }

    public async Task DispatchAsync(Session session, byte[] payload, CancellationToken cancellationToken)
    {
        ...
        catch (Exception ex)
        {
            ...
            var response = await FilterExceptionAsync(context, cancellationToken)
                .ConfigureAwait(false);

            await WriteFrameAsync(session, response, cancellationToken);
            return;
        }
        ...
        foreach (var handler in handlers)
        {
            try
            {
                var response = await ExecuteHandlerAsync(handler, session, command, request, cancellationToken);
                if (!response.IsEmpty)
                    tasks.Add(WriteFrameAsync(session, response, cancellationToken));
            }
            catch (Exception ex)
            {
