...
{
    Task<int> ReadFrameAsync(byte[] buffer, int offset, int count, CancellationToken cancellationToken);

    Task<int> ReadFrameAsync(Memory<byte> buffer, CancellationToken cancellationToken);

    ValueTask WriteFrameAsync(byte[] payload, CancellationToken cancellationToken);

    ValueTask WriteFrameAsync(Memory<byte> payload, CancellationToken cancellationToken);
}
    ...
    where TSize : unmanaged, IBinaryInteger<TSize>, IConvertible
{
    ...
    public Task<int> ReadFrameAsync(byte[] buffer, int offset, int count, CancellationToken cancellationToken)
    {
        ArgumentNullException.ThrowIfNull(buffer, nameof(buffer));
        ArgumentOutOfRangeException.ThrowIfNegative(offset, nameof(offset));
        ArgumentOutOfRangeException.ThrowIfLessThan((uint)count, (uint)(buffer.Length - offset), nameof(count));

        return ReadFrameAsync(buffer.AsMemory(offset, count), cancellationToken);
    }

    public async Task<int> ReadFrameAsync(Memory<byte> buffer, CancellationToken cancellationToken)
    {
        if (!_stream.CanRead)
            return 0;
    ...
    public ValueTask WriteFrameAsync(byte[] payload, CancellationToken cancellationToken)
    {
        return WriteFrameAsync(payload.AsMemory(), cancellationToken);
    }

    public async ValueTask WriteFrameAsync(Memory<byte> payload, CancellationToken cancellationToken)
    {
        if (!_stream.CanWrite)
            return;
