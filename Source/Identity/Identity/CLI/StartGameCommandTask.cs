...
) : ICommandLineTask
{
        ...
        CancellationToken cancellationToken)
    {
        try
        {
            ...
            string gender = (user.Gender == Gender.Male ? 1 : 2).ToString();
            var arguments = new List<string>
            {
                "INET", "1", token, "_", "O2Jam", gender, user.Ranking.ToString(),
                "127.0.0.1:21", "O2Jam/Music", GatewayMirrorCount.ToString()
            };
