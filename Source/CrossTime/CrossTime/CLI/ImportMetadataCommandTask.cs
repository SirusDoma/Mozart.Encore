...
public class ImportMetadataCommandTask(IOptions<MetadataOptions> metadataOptions) : ICommandLineTask
{
    ...
    private async Task<int> ExecuteAsync(string directory, CancellationToken cancellationToken)
    {
        try
        {
            ...
            MetadataFile? musicList = ReadStandaloneFile(imageDirectory, "X2OJNList.dat");
            if (musicList is { } music)
            {
                await WriteAsync(metadataOptions.Value.MusicList, "X2OJNList.dat", music, cancellationToken);
                imported++;
            }
            else
                Console.WriteLine("X2OJNList.dat is not found.");
