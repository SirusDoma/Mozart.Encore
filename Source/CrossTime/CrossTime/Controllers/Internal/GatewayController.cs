using CrossTime.Internal.Requests;
...
namespace CrossTime.Controllers.Internal;
    ...
    public async Task GrantSession(GrantSessionResponse response, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            await client.WriteFrame(codec.Encode(new ChannelLoginResponse
            {
                ...
                ErrorCode = response.Success ? null : LoginErrorCode.Undefined,
                Ranking   = response.Success ? response.Ranking : null
            }), cancellationToken);
        }
        catch (Exception ex)
        {
