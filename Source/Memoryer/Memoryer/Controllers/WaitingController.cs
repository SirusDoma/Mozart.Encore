...
) : CommandController<Session>(session)
{
    ...
    public async Task StartGame(CancellationToken cancellationToken)
    {
        ...
        if (skills.Any(i => i > 0) && !options.Value.InfinityRing && (!user.InfinityRing || (user.InfinityRing && (!user.InfinityRingExpiryDate.HasValue || user.InfinityRingExpiryDate.Value.ToUniversalTime() < DateTime.UtcNow))))
        {
