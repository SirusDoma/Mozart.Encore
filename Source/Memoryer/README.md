...
Supported client version: **v7.55 (O2Jam Classic)**
...
Download and extract the Memoryer server binary from [here](https://github.com/SirusDoma/Mozart.Encore/releases/latest).
...
```powershell
.\Memoryer.Encore.exe game:start <username> <password> "C:\Program Files (x86)\NOWCOM\O2Jam\"
```
...
| Project                                      | Description                                            |
|----------------------------------------------|--------------------------------------------------------|
| [Memoryer](Memoryer/)                        | Executable, controllers, messages, and CLI tasks       |
| [Memoryer.Web](Memoryer.Web/)                | HTTP host that compiles the shared `Encore.Web` source |
| [Memoryer.Server](Memoryer.Server/)          | Build-specific game and protocol logic                 |
| [Memoryer.Data](Memoryer.Data/)              | Build-specific EF Core data model and metadata parsers |
| [Memoryer.Migrations](Memoryer.Migrations/)  | Database migrations organized by provider              |
| [Memoryer.Relay](Memoryer.Relay/)            | Memoryer-only TCP/UDP relay servers for Live Mode      |
...
| `MusicList` | Path of `OJNList.dat` exclusive for this channel. Format must compatible with client v`7.55` (O2Jam Classic).  Default: (Empty) using global [Metadata](#Metadata)  |
| `ItemData`  | Path of `Itemdata.dat` exclusive for this channel. Format must compatible with client v`7.55` (O2Jam Classic).  Default: (Empty) using global [Metadata](#Metadata) |
...
| Option      | Description                                                                                                 |
|-------------|-------------------------------------------------------------------------------------------------------------|
| `MusicList` | Relative or absolute path of `OJNList.dat`. Format must compatible with client v`7.55` (O2Jam Classic).  |
| `ItemData`  | Relative or absolute path of `Itemdata.dat`. Format must compatible with client v`7.55` (O2Jam Classic). |
...
> See [AuthParameters](Memoryer/Utilities/AuthParameters.cs) and [AuthParameterRsaCipher](Memoryer/Utilities/AuthParameterRsaCipher.cs) for details about encoding and decoding `encrypted_parameters`.
>
> Use [`user:authorize`](#cli-commands) to generate the session and encrypted parameters.
...
- `encdec <param>`: Encrypt or decrypt authentication parameters.
