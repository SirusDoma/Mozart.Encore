...
) : CommandController<IRelayPeer>(session)
{
    ...
    public async Task Confirm(PeerConfirmRequest request, CancellationToken cancellationToken)
    {
        ...
        if (Session.Authorized)
        {
            ...
            if (actor.SessionKey1 != request.SessionKey1 || actor.SessionKey2 != request.SessionKey2 || !actor.RelayServer.Equals(Session.LocalEndPoint))
            {
                actor.RelayServer = Session.LocalEndPoint;
